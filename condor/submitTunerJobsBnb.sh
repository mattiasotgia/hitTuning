...
sourceDir="/pnfs/icarus/scratch/users/msotgia/hitTuning/mc/lhsGridBnb/"
...
export tarFile="TunerInstallBnb.tar.gz"         # Tarball with code and dependencies
...
jobsub_cmd="jobsub_submit -G icarus -N ${nJobs} --maxConcurrent=350 --expected-lifetime=50h --disk=100GB --memory=8000MB --blacklis=RAL -e IFDH_CP_MAXRETRIES=4 -e IFDH_CP_UNLINK_ON_ERROR=2 --lines '+FERMIHTC_AutoRelease=True' --lines '+FERMIHTC_GraceMemory=4096' --lines '+FERMIHTC_GraceLifetime=3600' -l '+SingularityImage=\"/cvmfs/singularity.opensciencegrid.org/fermilab/fnal-wn-sl7\:latest\"' --append_condor_requirements='(TARGET.HAS_Singularity==true)'"
