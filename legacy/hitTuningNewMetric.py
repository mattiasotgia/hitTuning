...
class ParamSpec:
    ...
    decimals: int = 2  # only used when kind='float'
...
class HitTuningDB:
    ...
    def create_tables(self) -> None:
        ...
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS runs (
                ...
                ideEnergy REAL,
                eventRatio REAL,
                plane0HitEnergy REAL,
                plane1HitEnergy REAL,
                plane2HitEnergy REAL,
                plane0IdeEnergy REAL,
                plane1IdeEnergy REAL,
                plane2IdeEnergy REAL,
                plane0Ratio REAL,
                plane1Ratio REAL,
                plane2Ratio REAL,
                protons INTEGER, 
                chargedPions INTEGER, 
                muons INTEGER,
                electrons INTEGER, 
                neutralPions INTEGER, 
                gammas INTEGER
            )
        ''')
    ...
    def add_run(self, params: fclParams, jobNum: int, fcl_filename: str, results: List[float], 
        ...
        cursor.execute(f'''
            INSERT INTO runs (
                ...
                plane0IdeEnergy, plane1IdeEnergy, plane2IdeEnergy,
                plane0Ratio, plane1Ratio, plane2Ratio,
                protons, chargedPions, muons, electrons, neutralPions, gammas
            ) VALUES ({placeholders})
        ''', (
            jobNum, timestamp, fcl_filename, output_filename, hist_filename,
            ...
            params.MaxMultiHit, params.Chi2NDF, notes, 
            event, subRun, run, 
            hitEnergy, ideEnergy, eventRatio, 
            plane0HitEnergy, plane1HitEnergy, plane2HitEnergy,
            plane0IdeEnergy, plane1IdeEnergy, plane2IdeEnergy,
...
def generateFhicl(params: fclParams, outputFile: str = "hitTuningMC.fcl", verbose: bool = False) -> None:
...
physics.producers.mcassociationsGausCryoE.HitParticleAssociations.HitModuleLabelVec: [
    "gaushit2dTPCEE",
    "gaushit2dTPCEW"
]

physics.producers.mcassociationsGausCryoW.HitParticleAssociations.HitModuleLabelVec: [
    "gaushit2dTPCWE",
    "gaushit2dTPCWW"
]

physics.producers.gaushit2dTPCEE.HitFinderToolVec.CandidateHitsPlane0.RoiThreshold:      {params.roiThreshold[0]}
physics.producers.gaushit2dTPCEE.HitFinderToolVec.CandidateHitsPlane0.NextADCThreshold: -0.15
physics.producers.gaushit2dTPCEE.HitFinderToolVec.CandidateHitsPlane1.RoiThreshold:      {params.roiThreshold[1]}
physics.producers.gaushit2dTPCEE.HitFinderToolVec.CandidateHitsPlane1.NextADCThreshold: -0.15
physics.producers.gaushit2dTPCEE.HitFinderToolVec.CandidateHitsPlane2.RoiThreshold:      {params.roiThreshold[2]}
physics.producers.gaushit2dTPCEE.HitFinderToolVec.CandidateHitsPlane2.NextADCThreshold: -0.15
...
physics.producers.gaushit2dTPCEW.HitFinderToolVec.CandidateHitsPlane0.NextADCThreshold: -0.15
physics.producers.gaushit2dTPCEW.HitFinderToolVec.CandidateHitsPlane1.RoiThreshold:      {params.roiThreshold[1]}
physics.producers.gaushit2dTPCEW.HitFinderToolVec.CandidateHitsPlane1.NextADCThreshold: -0.15
physics.producers.gaushit2dTPCEW.HitFinderToolVec.CandidateHitsPlane2.RoiThreshold:      {params.roiThreshold[2]}
physics.producers.gaushit2dTPCEW.HitFinderToolVec.CandidateHitsPlane2.NextADCThreshold: -0.15
...
physics.producers.gaushit2dTPCWE.HitFinderToolVec.CandidateHitsPlane0.NextADCThreshold: -0.15
physics.producers.gaushit2dTPCWE.HitFinderToolVec.CandidateHitsPlane1.RoiThreshold:      {params.roiThreshold[1]}
physics.producers.gaushit2dTPCWE.HitFinderToolVec.CandidateHitsPlane1.NextADCThreshold: -0.15
physics.producers.gaushit2dTPCWE.HitFinderToolVec.CandidateHitsPlane2.RoiThreshold:      {params.roiThreshold[2]}
physics.producers.gaushit2dTPCWE.HitFinderToolVec.CandidateHitsPlane2.NextADCThreshold: -0.15
...
physics.producers.gaushit2dTPCWW.HitFinderToolVec.CandidateHitsPlane0.NextADCThreshold: -0.15
physics.producers.gaushit2dTPCWW.HitFinderToolVec.CandidateHitsPlane1.RoiThreshold:      {params.roiThreshold[1]}
physics.producers.gaushit2dTPCWW.HitFinderToolVec.CandidateHitsPlane1.NextADCThreshold: -0.15
physics.producers.gaushit2dTPCWW.HitFinderToolVec.CandidateHitsPlane2.RoiThreshold:      {params.roiThreshold[2]}
physics.producers.gaushit2dTPCWW.HitFinderToolVec.CandidateHitsPlane2.NextADCThreshold: -0.15
...
#     v_LongPulseWidth = [[2.0], [5.0], [8.0], [10.0]]
...
def createGrid(defaultFirst: bool = True, n_samples: int = 200) -> List[fclParams]:
    try:
    ...
    continuous = {
        'roiThreshold':    (1.0,  6.0),
        'minPulseHeight':  (1.0,  4.0),
        'minPulseSigma':   (0.5,  2.0),
        'LongPulseWidth':  (2.0,  10.0),
        'PulseHeightCuts': (2.0,  4.0),
        'Chi2NDF':         (200., 2500.),
    }

    # Integer parameters — bounds are inclusive
    integer = {
        'LongMaxHits': (1,  15),
        'MaxMultiHit': (3,  15),
    }

    cont_keys = list(continuous.keys())
    int_keys  = list(integer.keys())
    all_keys  = cont_keys + int_keys

    cont_lows  = [continuous[k][0] for k in cont_keys]
    cont_highs = [continuous[k][1] for k in cont_keys]
    int_lows   = [integer[k][0]    for k in int_keys]
    int_highs  = [integer[k][1]    for k in int_keys]

    # One LHS sampler covering all dimensions
    sampler = qmc.LatinHypercube(d=len(all_keys), seed=42)
    raw = sampler.random(n=n_samples)          # shape (n_samples, n_dims), values in [0, 1)

    # Scale continuous dims normally
    cont_scaled = qmc.scale(raw[:, :len(cont_keys)], cont_lows, cont_highs)

    # Scale integer dims: map [0,1) → [low, high] as integers
    int_raw = raw[:, len(cont_keys):]
    int_scaled = np.floor(
        int_raw * np.array([h - l + 1 for l, h in integer.values()])
    ).astype(int) + np.array(int_lows)

    param_grid = []
    for i in range(n_samples):
        cont_vals = dict(zip(cont_keys, cont_scaled[i]))
        int_vals  = dict(zip(int_keys,  int_scaled[i]))

        params = fclParams(
            roiThreshold    = round(float(cont_vals['roiThreshold']), 2),
            minPulseHeight  = round(float(cont_vals['minPulseHeight']), 2),
            minPulseSigma   = round(float(cont_vals['minPulseSigma']), 2),
            LongPulseWidth  = round(float(cont_vals['LongPulseWidth']), 1),
            PulseHeightCuts = round(float(cont_vals['PulseHeightCuts']), 1),
            Chi2NDF         = round(float(cont_vals['Chi2NDF']), 1),
            LongMaxHits     = int(int_vals['LongMaxHits']),
            MaxMultiHit     = int(int_vals['MaxMultiHit']),
    ...
    if defaultFirst:
        param_grid.insert(0, fclParams())

    print(f"Created LHS grid with {len(param_grid)} samples")
    return param_grid
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('-b', '--batches', type=int, default=10, help='Process files in batches')
...
if __name__ == "__main__":
    ...
    if args.createGrid:
        ...
        for ip, params in enumerate(paramGrid):
            ...
            generateFhicl(params, outputFile=outputFCL, verbose=args.verbose)
    ...
    if args.runGrid:
        ...
        abs_macro = os.path.join(macroPath, 'hitScorer.C')
        ...
        if not os.path.exists(abs_macro):
            raise FileNotFoundError(
                f"hitScorer.C not found at {abs_macro}. "
        ...
        with open(inputFile, "r") as f:
            ...
            for batch_idx, batch in enumerate(chunk_list(filenames, args.batches)):
                # Create temporary file
                with tempfile.NamedTemporaryFile(mode="w", delete=False) as tmp:
                    tmp.write("\n".join(batch))
                    tmp_filename = tmp.name
                
                print(f"Processing batch {batch_idx} with {len(batch)} files")
                # batching in multiple files...
                try:
                    run(fclFile, tmp_filename, outputFile, options='-n-1')

                    histFile = f'hist_output_{jobNum}.root'
                    print(f"Processing hits with outputFile: {outputFile} and histFile: {histFile}")
                    results = r.hitScorer(outputFile, histFile)
                    for i, result in enumerate(results):
                        print(len(result))
                        run_id = db.add_run(params, jobNum, fclFile, result, outputFile, histFile, notes="")
